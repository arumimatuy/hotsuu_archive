...
function App() {
  ...
  // Derived list for AuthorList (Active Kana Row Authors)
  const availableAuthors = useMemo(() => {
    if (selectedKana === 'all') return []

    // Logic similar to above but solely for extracting authors
    const kanaMap = {
  ...
  const filteredBooks = useMemo(() => {
    ...
    // Kana Filter (Row level e.g., 'ka' matches ka, ki, ku, ke, ko)
    if (selectedKana !== 'all') {
      // Define the mapping for Rows
      const kanaMap = {
        'あ': ['あ', 'い', 'う', 'え', 'お', 'ア', 'イ', 'ウ', 'エ', 'オ'],
        'か': ['か', 'き', 'く', 'け', 'こ', 'が', 'ぎ', 'ぐ', 'げ', 'ご', 'カ', 'キ', 'ク', 'ケ', 'コ', 'ガ', 'ギ', 'グ', 'ゲ', 'ゴ'],
        'さ': ['さ', 'し', 'す', 'せ', 'そ', 'ざ', 'じ', 'ず', 'ぜ', 'ぞ', 'サ', 'シ', 'ス', 'セ', 'ソ', 'ザ', 'ジ', 'ズ', 'ゼ', 'ゾ'],
    ...
    return result
  }, [books, searchQuery, sortBy, selectedKana])
